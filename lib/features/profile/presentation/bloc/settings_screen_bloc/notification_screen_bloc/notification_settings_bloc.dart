...
class NotificationSettingsBloc
  ...
  NotificationSettingsBloc() : super(const NotificationSettingsState()) {
    ...
    on<ToggleListingStatus>((event, emit) =>
        emit(state.copyWith(listingStatus: event.value)));
    on<ToggleListingReminders>((event, emit) =>
        emit(state.copyWith(listingReminders: event.value)));
    on<ToggleNewMessage>((event, emit) =>
        emit(state.copyWith(newMessage: event.value)));
    on<ToggleResponseReminders>((event, emit) =>
        emit(state.copyWith(responseReminders: event.value)));
    on<ToggleDiscovery>((event, emit) =>
        emit(state.copyWith(discovery: event.value)));
    on<ToggleSystem>((event, emit) => emit(state.copyWith(system: event.value)));
  }
}
